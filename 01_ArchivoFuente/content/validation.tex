\begin{center}
        \vspace*{-24pt}
        \includegraphics[width=0.35\textwidth]{images/logoUSM.pdf}
        
    \large\textbf{CONSTANCIA DE VALIDACIÓN Y CONFIDENCIALIDAD DE MONOGRAFÍA A REPOSITORIO ACADÉMICO}
\end{center}


\noindent \textbf{\small 1.- IDENTIFICACIÓN DEL TRABAJO ACADÉMICO}

\vspace{0.1cm}
{\small
\noindent \textbf{Tipo de monografía (marcar una opción):} \hspace{0.5cm} \checkbox\ Memoria o trabajo de título \hspace{0.5cm} \checkedbox\ Tesis de Postgrado\\
\textbf{Título del trabajo:}  Design and Sizing of an Energy Storage System for a Hybrid Tugboat\\
\textbf{Nombre del candidato(a):} Leonardo Solis Zamora \\
\textbf{Carrera / Grado:} Magíster en Ciencias de la Ingeniería Electrónica  \\
\textbf{Campus:} Casa Central Valparaíso \hspace{4.5cm} \textbf{Departamento:} Electrónica
}
\vspace{0.3cm}

 \noindent \textbf{\small 2.- VALIDACIÓN DEL PROFESOR GUÍA/DIRECTOR DE TESIS}

\vspace{0.1cm}
{\small 
Yo, Marcelo Pérez Leiva, en mi calidad de profesor(a) guía/director(a) del trabajo académico mencionado anteriormente \textbf{DEJO CONSTANCIA} que:

\begin{itemize}
\item He revisado esta versión del documento y corresponde a la versión final aprobada del trabajo.
\item El trabajo cumple con los requisitos académicos y de formato establecidos por la institución.
\end{itemize}
}
 \noindent \textbf{\small 3.- EVALUACIÓN DE CONFIDENCIALIDAD POR PROPIEDAD INDUSTRIAL (marcar una opción)}

\vspace{0.1cm}
{\small 
\noindent \checkbox\ El trabajo \textbf{NO} contiene información que amerite confidencialidad y puede ser publicado de inmediato en repositorio con acceso abierto.\\
\checkedbox\  El trabajo \textbf{CONTIENE} información con potenciales implicancias de propiedad industrial o intelectual y requiere un periodo de confidencialidad (embargo) por (marcar una opción):

\vspace{0.2cm}
\checkbox\ 6 meses \hspace{0.5cm}
%$\boxtimes$ 12 meses \hspace{0.5cm}
\checkedbox\ 12 meses \hspace{0.5cm}
\checkbox\ 2 años \hspace{0.5cm}
\checkbox\ 3 años \hspace{0.5cm}
\checkbox\ 5 años \hspace{0.5cm}
\checkbox\ 10 años

\vspace{0.2cm}

\noindent Fundamentación de la necesidad de confidencialidad (obligatorio si se solicita embargo):\\
\\
\hspace{4.5cm} \underline{Uso de información en AC3E para proyectos con empresas del rubro marítimo.}\\
}

 \noindent \textbf{\small 4.- FIRMAS}

\vspace{0.1cm}
{\small
\noindent \textbf{Profesor(a) guía o director(a) de memoria o tesis:}\\[0.2cm] 
\indent \hspace{4.5cm} \textbf{Fecha:} \underline{6-3-2026} \hspace{2cm} \textbf{Firma:} %\underline{\textbf{Marcelo Pérez Leiva}}

\vspace{2.5cm}

\noindent \textbf{Estudiante o Candidato(a):}\\[0.2cm]
\indent \hspace{4.5cm} \textbf{Fecha:} \underline{6-3-2026} \hspace{2cm} \textbf{Firma:} 
}

%\vspace{0.3cm}
%\begin{wrapfigure}{r}{0.5\textwidth}
%\includegraphics[scale=0.08]{images/Firma_LSZ.png}    
%\end{wrapfigure}

\vspace{1cm}

\noindent \textit{\small Este formulario debe ser insertado como página 2 de la memoria o tesis, completado y firmado por estudiante y profesor(a) antes de la entrega en portal PRISMA de Biblioteca USM.}\\%
